...
import { formatOctets, resumeOrphelines } from './photosLogique'

// Recherche puis suppression des fichiers photo qu'aucun compte rendu
...
export function NettoyageStockage({ onTermine }) {
  ...
  const supprimer = async () => {
    setEtat('suppression')
    try {
      await supprimerFichiers(fichiers.map(f => f.chemin))
  ...
  return (
    <div style={{ fontSize: 11, color: '#5E5854', marginTop: 10 }}>
      ...
      {etat === 'recherche' && <span>Recherche des photos inutilisées…</span>}
      {etat === 'resultat' && (resume.fichiers === 0 ? (
        <span>Aucune photo inutilisée. <button type="button" onClick={() => setEtat('repos')} style={{ ...lien, color: '#5E5854' }}>Fermer</button></span>
      ) : (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span>
            <strong style={{ color: '#1F1B17' }}>{resume.photos} photo{resume.photos > 1 ? 's' : ''} inutilisée{resume.photos > 1 ? 's' : ''}</strong>
      ...
      {etat === 'fini' && bilan && (
        <span style={{ color: '#2A8A4E' }}>
          {bilan.photos} photo{bilan.photos > 1 ? 's' : ''} supprimée{bilan.photos > 1 ? 's' : ''}, {formatOctets(bilan.taille)} libérés.
        </span>
      )}
    </div>
  )
